...
| `--u-max derived\|<int>` | `derived` | ALP specificity ceiling. `derived` = `a + 2`, which is **too tight at k=135** — use 11 there. |
...
Diagnostics, all off by default and read-only over the population:
`--log-trajectory` (the S-curve), `--log-accuracy` (the metric the literature reports),
`--log-coverage` (**start here when knowledge sticks at 0.75 or 0.50** — it means whole
classes of transition are empty), `--log-quadrant-detail`, `--log-diagnostics`.
...
| Crate | Purpose |
|---|---|
| `acs2-core` | Classifier, population, ALP, RL, GA, action selection, injected RNG, and both agents behind the `trial::LearningAgent` trait. No I/O, no environment knowledge. |
| `acs2-envs` | The `Environment` trait: 8-sensor maze (27 geometries in `mazes/`) and the multiplexer with both encodings. |
...
Runs are deterministic from an injected RNG: same seed and configuration reproduce a run
trial for trial, across architectures. Trials-to-success is therefore the
machine-independent metric; wall-clock is not.
